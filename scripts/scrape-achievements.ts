...
const CACHE_PATH = path.resolve(__dirname, '../prisma/data/achievements-cache.json');
...
function updateVersionFile(versionNumber: string, categories: AchievementCategory[]) {
  ...
  console.log(`✅ Updated ${versionNumber}.json (${categories.reduce((acc, c) => acc + c.achievements.length, 0)} achievements)`);
}
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage:');
    console.error('  Fetch all + update versions : npx ts-node ... scrape-achievements.ts --fetch 1.0 1.1 3.5');
    console.error('  Use cache + update versions : npx ts-node ... scrape-achievements.ts --cache 1.0 1.1 3.5');
    process.exit(1);
  }
  ...
  if (useCache) {
    ...
    console.log('Fetching all achievements from wiki (this will take a few minutes)...');
